...
- 10g 设计稿视觉对齐（用户反馈后补齐）：①②③④ 工作区序号徽章（ZoneBadge 自绘控件，陶土主题色）；命名设置分区标题；预览标题并入工具条（3 行壳→2 行）；"新文件名末尾"+"自定义"合并为"自定义末尾"勾选框（文案合并，悬浮说明保留）；执行中条——操作期间窗口最底部整条"执行中：[进度条] N% [取消]"（替代挤在执行栏里的小进度条）。截图三态核对（浅/深/执行中 62%）。**有意偏差**：护眼/关于按钮留在设置行（设计稿的标题栏图标需自绘窗口框架）；表格保留 A/B/C 字母列前缀（提示文案与逐行场号换列依赖它）；徽章用陶土主题色而非设计稿蓝。
- SmokeTest 同提交扩展：执行栏结构、进度初始隐藏、折叠往返断言。
- 10h 设计稿严格对齐（用户复核后二次补齐，标注：视觉/文案/行为变更）：列头去掉 A/B/C 字母前缀（10g 记录的该项偏差就此撤销，全部提示文案改用列名）；场号列常显（逐行场号关闭时只读显示全局值并就地同步，开启时可编辑）；进度列常显（0% 留白，空闲表格不排满"0%"条）；隐藏行头列；空素材格"拖入视频..."弱化占位（仅显示层替换，底层 Value 不动，网格等价性转储不受影响）；执行中条换主题化胶囊进度条（SlimProgressBar，替换系统绿色条）+ 文案"N% · 当前文件名"（导出=目标名、重命名=原名）；信息面板改题"素材信息"，并修复 Fill/Right 停靠重叠——面板原先靠 Z 序悬浮在列表右缘之上，列表最右列与滚动条被压在面板底下（DrawToBitmap 截图反序漏画由此暴露的真实缺陷）；勾选框"导出1080×1920"；空闲就绪态状态栏"就绪 · N 条待重命名"。冒烟新增：设计稿网格结构锁定 + 执行中条文案/复位锁定。**剩余有意偏差**：护眼/关于按钮留在设置行（设计稿的标题栏图标需自绘窗口框架）；预览列表保留 8 列（行/镜号/末尾/列/信息承载冲突诊断与批量末尾编辑，删列即删信息）；徽章/主按钮用陶土主题色而非设计稿蓝（设计稿蓝为 mockup 工具默认色，暖纸色系为用户确认偏好）。

- 10i 审计补齐（10 审计员并行逐项核对 + 吹毛求疵评审工作流；34/35 代码断言经 file:line 证据确认）：两处设计稿偏差补齐——**预览状态列文字着色**（就绪/待执行绿、阻塞红、未变化弱化；暖纸系新增 PreviewOkFore/PreviewErrorFore 角色并纳入色值锁定用例；逐子项配色须关 UseItemStyleForSubItems 并显式铺每列底/前景）；**素材信息卡分辨率行并入时长**（"分辨率：1920 x 1080 · 8s"，Shell 时长列 + 帧宽高同款列号缓存，整秒精度——设计稿 "8.2s" 的十分位 Shell 不提供；时长列值须形如 H:MM:SS 才被采纳/缓存，防 "Focal length" 误命中；预览"信息"列同步带时长）。文档纠偏：健康评估"依赖方向全机器强制"降为如实的"UI 框架纯净门禁"（审计证伪点：Services/Media 引用 App 层 AppInfo，命名空间门禁不查跨层类型引用；AppInfo 仅常量，候选迁 Core）+ 行数/分部数刷新（Ui.cs 734 非空行、12 分部）。**新增有意偏差记录**：素材信息卡保留多行块（目标文件名/大小/修改时间承载操作信息，非设计稿单行）；主按钮勾选导出后文案切换"导出1080p"（10g 前既有的功能性行为，设计稿未覆盖该态）；按钮宽度按标称值 0.8 折算（紧凑化，观感待人工高 DPI 复核）。
- SmokeTest/SelfTest 同提交扩展：状态列色值 4 枚新钉进 palette_pins 两用例（仍 70 用例）。
